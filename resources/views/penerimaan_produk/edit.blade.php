...
@section('title', 'Edit Penerimaan Produk')
        ...
        <div class="col-md-10">
            <div class="card shadow-sm">
                <div class="card-header bg-warning text-dark">
                    <h4 class="mb-0 text-center">Edit Penerimaan Produk</h4>
                </div>
                <div class="card-body p-4">

                    <form action="{{ route('penerimaan_produk.update', $item->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="faktur" class="form-control" 
                                       value="{{ old('faktur', $item->faktur) }}" required>
                            ...
                            <div class="col-md-6">
                                ...
                                <small class="text-muted d-block mt-2">
                                    ...
                                    <span id="info_minggu" class="text-primary fw-bold">
                                        {{ $item->tanggal ? 'Minggu ' . min(ceil(\Illuminate\Support\Carbon::parse($item->tanggal)->day / 7), 5) : 'Pilih tanggal' }}
                                    </span>
                                </small>
                            </div>
                                ...
                                <select name="label" id="label" class="form-select" required>
                                    ...
                                    @foreach($produks as $p)
                                        <option value="{{ $p->label }}"
                                                ...
                                                data-isi="{{ $p->isi ?? '' }}"
                                                {{ old('label', $item->label) == $p->label ? 'selected' : '' }}>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" name="harga" id="harga" class="form-control text-end" min="0" 
                            ...
                            <!-- Status & Isi (Otomatis dari produk, bisa diubah manual) -->
                            <div class="col-md-6">
                                <label class="form-label fw-bold">Status</label>
                                <input type="text" name="status" id="status" class="form-control" 
                                       value="{{ old('status', $item->status) }}" placeholder="Otomatis dari produk">
                            ...
                            <div class="col-md-6">
                                ...
                                <textarea name="isi" id="isi" class="form-control" rows="3" 
                                          placeholder="Otomatis dari produk">{{ old('isi', $item->isi) }}</textarea>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function hitungMinggu() {
        if (!inputTanggal.value) {
            infoMinggu.textContent = 'Pilih tanggal';
            return;
        }
    ...
    selectLabel.addEventListener('change', function () {
        ...
        if (option.value) {
            ...
            // Status & Isi otomatis dari produk (tapi tidak override nilai existing kalau sudah diubah manual)
            if (!statusInput.dataset.changed) {
                statusInput.value = option.dataset.status || '';
            }
            if (!isiTextarea.dataset.changed) {
            ...
            if (!statusInput.dataset.changed) statusInput.value = '';
            if (!isiTextarea.dataset.changed) isiTextarea.value = '';
        }
        hitungTotal();
    });

    // Tandai kalau user ubah manual status/isi
    ...
    // Trigger change pada produk yang sudah dipilih
